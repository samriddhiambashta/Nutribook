...
class DatabaseHandlersStep {
    ...
    return openDatabase(
      ...
      onCreate: (database, version) async {
        await database.execute(
          "CREATE TABLE stepsTrack(id INTEGER PRIMARY KEY AUTOINCREMENT, steps INTEGER,  dateTime TEXT)",
        );
      },
      version: 1,
    );
  ...
  // inserted row.
  Future<int> insertUser(List<StepInfo> users) async {
    int result = 0;
    final Database db = await initializeDB();
    for (var user in users) {
      result = await db.insert('stepsTrack', user.toMap());
    }
    return result;
  }
  ...
  Future<List<StepInfo>> retrieveUsers() async {
    final Database db = await initializeDB();
    final List<Map<String, Object?>> queryResult =
        await db.query('weightTrack');
    return queryResult.map((e) => StepInfo.fromMap(e)).toList();
  }
  ...
  Future getSteps(DateTime dt) async {
    final db = await initializeDB();
    var newFormat = DateFormat("yy-MM-dd");
    String date = newFormat.format(dt);
    var result = await db.rawQuery(
        "SELECT steps  from stepsTrack where dateTime=? ORDER BY id DESC LIMIT 1",
        [date]);
    print(result);
